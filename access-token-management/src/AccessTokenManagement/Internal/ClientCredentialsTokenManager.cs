...
) : IClientCredentialsTokenManagementService
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        catch (PreventCacheException ex)
        {
            ...
            logger.WillNotCacheTokenResultWithError(clientName, ex.Failure.Error, ex.Failure.ErrorDescription);
            return ex.Failure;
        }
        catch (Exception ex) when (!ex.Data.Contains(ThrownInsideFactoryExceptionKey))
        {
            ...
            logger.ExceptionWhileReadingFromCache(ex, clientName);
    ...
    private async Task<ClientCredentialsToken> RequestToken(ClientCredentialsCacheKey cacheKey,
        ClientCredentialsClientName clientName, TokenRequestParameters parameters, CancellationToken cancellationToken)
    {
            ...
            && token.Expiration != DateTimeOffset.MaxValue)
        {
            ...
            logger.CachingAccessToken(clientName, token.Expiration);
        }
