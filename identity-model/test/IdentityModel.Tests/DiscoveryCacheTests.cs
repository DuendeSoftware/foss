...
public class DiscoveryCacheTests
{
    ...
    public async Task New_initialization_with_no_authority_and_client_func_without_base_address_should_throw()
    {
        ...
        var exception = await Should.ThrowAsync<ArgumentException>(async () => await cache.GetAsync());

        exception.Message.ShouldBe("Either the DiscoveryDocumentRequest Address or the HttpClient BaseAddress must not be null.");
    }
}
